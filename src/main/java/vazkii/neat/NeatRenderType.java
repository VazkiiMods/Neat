...
public class NeatRenderType extends RenderStateShard {
	...
	public static RenderType getHealthBarType(ResourceLocation location) {
		RenderType.CompositeState renderTypeState = RenderType.CompositeState.builder().setTextureState(new TextureStateShard(location, false, false)).setTransparencyState(TRANSLUCENT_TRANSPARENCY).setLightmapState(LIGHTMAP).createCompositeState(true);
		return RenderType.create("neat_health_bar", POSITION_TEX_COLOR_NORMAL_LIGHTMAP, VertexFormat.Mode.QUADS, 256, true, true, renderTypeState);
	}
}
